...
def map_metric_filter(group):
	...
	for i,index in enumerate(correct_candidates.index):
		ap+=1.0* (i+1) /(index+1)
	#print( ap/len(correct_candidates))
	return ap/len(correct_candidates) < 0.5
if __name__ =="__main__":
	
	result_dir = sys.argv[1]
	time = sys.argv[2]
	print("result_dir",result_dir,"time",time)
	test_file = os.path.join("data/{}".format(result_dir),"dev.tsv")
	test=pd.read_csv(test_file,header=None,sep="\t",names=["question","answer","flag"],quoting =3)
	#test_score = pd.read_csv("/ceph_ai/zhansu/bert_distill/output/{}/{}/test_results.tsv".format(time,result_dir),sep = "\t",names = ["negative","positive"])
	test_score=pd.read_csv('pytorch',sep='\t',names = ['negative','positive'],quoting = 3)
	print(evaluationBypandas(test,test_score['positive'].to_list()))
	print(evaluationBypandas(test,np.random.randn(len(test))))
	#test_score=pd.read_csv('pytorch',sep='\t',names = ['question','answer','flag'],quoting = 3)
	# train=pd.read_csv(train_file,header=None,sep="\t",names=["question","answer","flag"],quoting =3)
	# train['score'] = np.random.randn(len(train))
	# df_group = train.groupby('question').filter(mrr_metric_filter)
